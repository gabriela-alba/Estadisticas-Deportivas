...
const puntosEquipo = (registro, equipo) => {
  ...
  registroEquipo.forEach(anotacion => {
    ...
    puntos[apellido] = puntos[apellido] || 0 // El jugador está registrado?
...
const goleadorXequipo = (equipo) => {
  ...
  const goleadorA = [] = goleadorXequipo(puntosEquipoA) // Se obtiene el goleador del equipo A
  const goleadorB = [] = goleadorXequipo(puntosEquipoB) // Se obtiene el goleador del equipo B
...
const distribucionDePuntaje = () => {
  ...
  array.forEach(i => {
    ...
    cantTriple = (array.length - 1) - cantDoble;
